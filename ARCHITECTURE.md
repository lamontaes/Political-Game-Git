...
- The source substrate holds evidence about the real world. The simulation
  ...
  Its production corpora now include a headless national judicial-office
  identity, selection, vacancy, tenure, and renewal domain. That corpus is
  evidence only: it has no simulation adapter and creates no court behavior,
  player action, ideology, or evaluation.
  ...
  observations for analyst and presentation read models. It neither writes the
  World nor publishes news; a World-observation projection fails unavailable
  when the locked product has no distinct release date rather than treating an
  observation period as publication evidence. `src/simulation/`, `src/presentation/`,
...
- See `docs/systems/source-substrate.md`,
  `docs/systems/judicial-office-selection.md`, and decisions D-074 and D-077.
...
World schema version 15, generator version `demo-world-v15`, person-materialization version 4, policy-, mind-, world-metric-, causal-mechanism-, incident-definition-, and vitality-catalog versions, and snapshot format version 14 are stored explicitly. Run D-Lite advances the world and snapshot boundaries for canonical moments, scheduled activities, and office work; the accepted Stage 6 catalogs remain unchanged. Person creation separately retains `person-v5` / `names-v1` for generated worlds and `demo-person-v4` / `demo-names-v4` for the compatibility demo; materialization is still version 4. Older world and envelope versions are rejected because no migration chain is promised yet. This build promises same-version reproducibility only and does not fabricate migration sequence for legacy biography facts.
