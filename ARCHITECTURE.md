...
World schema version 12, generator version `demo-world-v12`, person-materialization version 4, policy-, mind-, world-metric-, and causal-mechanism-catalog versions, and snapshot format version 11 are stored explicitly. Stage 6 Run C advances the world and snapshot boundaries because six quantitative-policy history families are persistable; metric catalog v2 and causal catalog v1 remain unchanged. Older world and envelope versions are rejected because no migration chain is promised yet. This build promises same-version reproducibility only and does not fabricate migration sequence for legacy biography facts.
...
Stage 6 Run C keeps descriptive proposition metadata separate from exact quantitative policy truth. An alternative owns one or more closed typed operations over frozen baseline revisions, explicit metric/scope/period, exact magnitude, and five independently inspectable implementation factors. An estimate and shared projected causal root create no activation or metric truth. Only a separate realization may create actual Run B causes/effects, and only later explicit aggregate evaluation may write canonical metric state. Policy analysis becomes subjective through an ordinary per-person event-knowledge path; the decision adapter consumes actor interpretation without an ideology or feasibility score. See [Quantitative Policy Semantics, Baselines, and Implementation](docs/systems/quantitative-policy.md).
...
The pure simulation exposes a versioned JSON snapshot codec that validates stable identity, entity ordering, policy-, mind-, metric-, and causal-mechanism-catalog definitions, control references, biography and life-graph invariants, political/mind/life chronology and provenance, lifecycle and supersession chains, typed life-source availability, exact quantity/unit and money/resource integrity, metric correction and observation-vintage predecessor chronology, causal ancestry/effect timing, typed magnitude-period compatibility, and source availability, future due-item references/closed state/strict-overdue boundary/outcomes, dwelling/occupancy/tenure references, historical cutoffs, decision-source snapshots, and contiguous history sequence at the persistence boundary. A Node-only `SqliteWorldRepository` stores the complete validated snapshot in a strict SQLite table and supports save, load, update, and list operations. SQLite code remains outside `src/simulation/`, so headless domain execution and browser diagnostics do not depend on a storage driver.
