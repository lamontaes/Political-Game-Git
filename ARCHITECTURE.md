...
World schema version 11, generator version `demo-world-v11`, person-materialization version 4, policy-, mind-, world-metric-, and causal-mechanism-catalog versions, and snapshot format version 10 are stored explicitly. Stage 6 Run B advances the world and snapshot boundaries because causal-process/effect-activation histories and the causal catalog are persistable; the metric catalog advances to v2 to distinguish primitive state from derived-only definitions. Older world and envelope versions are rejected because no migration chain is promised yet. This build promises same-version reproducibility only and does not fabricate migration sequence for legacy biography facts.
