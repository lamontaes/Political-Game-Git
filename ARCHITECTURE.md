...
World schema version 8, generator version `demo-world-v8`, person-materialization version 4, policy- and mind-catalog versions, and snapshot format version 7 are stored explicitly. The Run B version advances because `generated` life provenance and canonical history-plan content are newly persistable. Older world and envelope versions are rejected because no migration chain is promised yet. This build promises same-version reproducibility only and does not fabricate migration sequence for legacy biography facts.
