...
World schema version 13, generator version `demo-world-v13`, person-materialization version 4, policy-, mind-, world-metric-, causal-mechanism-, and incident-definition-catalog versions, and snapshot format version 12 are stored explicitly. Stage 6 Run D advances the world and snapshot boundaries because incident identity, state, and transition-plan history are persistable; metric catalog v2 and causal catalog v1 remain unchanged. Older world and envelope versions are rejected because no migration chain is promised yet. This build promises same-version reproducibility only and does not fabricate migration sequence for legacy biography facts.
