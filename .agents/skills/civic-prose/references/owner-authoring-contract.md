# Preserved owner authoring contract

This retains the complete prior prose contract and references. The current SKILL.md routes the workflow; the owner style, semantic grounding and holdout protections below remain. A procedural numbered order is not a requirement to redo already-accepted checks on unchanged text.
