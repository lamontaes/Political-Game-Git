...
/** What population a fraction or fixed count is measured against. */
export type VoteDenominatorBasis =
  | "TOTAL_MEMBERSHIP"
  | "MEMBERS_PRESENT"
  | "MEMBERS_PRESENT_AND_VOTING"
  | "FIXED_COUNT"
  | "OTHER";

/** Exact vote arithmetic where the source establishes it. */
export interface VoteThreshold {
  readonly numerator: number;
  readonly denominator: number;
  readonly denominatorBasis: VoteDenominatorBasis;
  /** A statutory fixed minimum, or null when it is derived at vote time. */
  readonly fixedVotesRequired: number | null;
}

/** One power's actual rule; a form label never manufactures this object. */
...
export interface EnumeratedPower {
  ...
  /** Target, conditions, exceptions and exact vote arithmetic, where sourced. */
  readonly details: Sourced<PowerRule>;
}
...
export type ElectionPartisanship = "PARTISAN" | "NONPARTISAN";

export type PresidingContext =
  "REGULAR_MEETING" | "SPECIAL_MEETING" | "WORK_SESSION";

export type LegislativeVoteRole = "FULL_VOTE" | "TIE_BREAKER_ONLY" | "NO_VOTE";

/** Who presides and votes in one defined meeting context. */
export interface PresidingRule {
  readonly context: PresidingContext;
  readonly presidingRole: ActorRole;
  readonly legislativeVoteRole: LegislativeVoteRole;
  readonly conditions: readonly string[];
}

/** Election timing kept as an explicit rule rather than parsed prose. */
export interface ElectionCalendarRule {
  readonly primaryHeld: boolean;
  readonly primaryTiming: string | null;
  readonly generalTiming: string;
  readonly electionYears: string;
}
...
export interface ElectedStructure {
  ...
  readonly presidingRules: readonly Sourced<PresidingRule>[];
  /** May contain HISTORICAL and KNOWN entries for a dated transition. */
  readonly partisanshipHistory: readonly Sourced<ElectionPartisanship>[];
  readonly electionCalendar: Sourced<ElectionCalendarRule>;
...
export interface AdministrativeStructure {
  ...
  readonly departmentHeadAuthority: Sourced<string>;
  readonly reportingRelationships: Sourced<string>;
}
...
export interface BudgetDeadlineRule {
  readonly monthDay: string;
  readonly minimumDaysBeforeFiscalYear: number | null;
}

/** Budget authority is separate from generic executive power. */
export interface BudgetProcedure {
  readonly fiscalYear: Sourced<FiscalYearRule>;
  readonly prepares: Sourced<readonly ActorRole[]>;
  readonly proposes: Sourced<ActorRole>;
  readonly amends: Sourced<ActorRole>;
  readonly adopts: Sourced<ActorRole>;
...
export interface Consolidation {
  ...
  readonly nestedGovernmentCount: Sourced<number>;
  readonly parallelGeneralGovernment: Sourced<string>;
}

export type MeetingPlaceKind =
  "REGULAR_MEETING" | "WORK_SESSION" | "MAYOR_OFFICE" | "MANAGER_OFFICE";
