...
export interface EmploymentRecord {
  ...
  /** Null whenever `fullTimeEquivalentResolution` reports an unresolved status. */
  readonly fullTimeEquivalentEmployees: number | null;
  readonly fullTimeEquivalentResolution: FteResolution;
...
/**
 * Why a full-time-equivalent figure is or is not available.
 *
 * FTE is a DERIVED quantity: full-time headcount plus the full-time equivalent
 * of part-time labour. Deriving the second term needs a conversion input —
 * part-time hours, or part-time payroll against an average full-time salary.
 *
 * When part-time employees exist and no conversion input is published, the
 * honest answer is that FTE is unknown. It is emphatically NOT the full-time
 * headcount: that figure omits the part-time contribution entirely while
 * presenting itself as a complete total. This type exists so that omission
 * cannot be made silently.
 */
export type FteResolutionStatus =
  /** The source published FTE directly. */
  | "reported_by_source"
  /** Derived from part-time hours. */
  | "derived_from_part_time_hours"
  /** Derived from part-time payroll and average full-time salary. */
  | "derived_from_part_time_payroll"
  /** No part-time employees exist, so FTE equals full-time headcount. */
  | "equals_full_time_no_part_time_staff"
  /** Part-time staff exist but no conversion input was published. */
  | "unresolved_missing_part_time_conversion_inputs"
  /** Part-time headcount itself is unknown, so FTE cannot be bounded. */
  | "unresolved_unknown_part_time_headcount"
  /** Full-time headcount is unknown, so no FTE base exists. */
  | "unresolved_unknown_full_time_headcount"
  /** An aggregate whose contributing records are not all resolved. */
  | "unresolved_incomplete_aggregate"
  /**
   * A stored record carries no FTE figure and no account of why. Older corpora
   * predate this metadata; they are treated as unresolved rather than assumed
   * complete.
   */
  | "unresolved_missing_resolution_metadata";

export interface FteResolution {
  readonly status: FteResolutionStatus;
  /** Plain-language account of what was and was not established. */
  readonly explanation: string;
}

export function isFteResolved(resolution: FteResolution): boolean {
  return !resolution.status.startsWith("unresolved_");
}
