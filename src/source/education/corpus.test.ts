...
import { generateQuickCharacterHistory } from "../../simulation/character-history.js";
import { createRunAFixture } from "../../presentation/run-a-fixture.js";
import {
...
import { loadEducationCorpusFromFile } from "./loader.node.js";
import type { CreateOrganizationInput } from "../../simulation/life.js";
import type { EducationCorpusSnapshot } from "./types.js";
...
describe("Education Corpus & NCES/IPEDS Source Module", () => {
  it("loads and validates the compiled production corpus snapshot cleanly", () => {
    ...
    expect(corpus.corpusScope).toBe(
      "sample-2022-vintage-not-national-universe",
    );
  ...
  it("proves raw artifact hashes in manifest match actual raw zip byte hashes if downloaded", () => {
    ...
    for (const artifact of corpus.rawArtifacts) {
      expect(artifact.rawFilename).toBeTruthy();
      expect(artifact.sha256).toMatch(/^[a-f0-9]{64}$/);
      expect(artifact.authoritativeUrl).toContain("nces.ed.gov");

      const zipPath = path.join(rawDir, artifact.rawFilename);
  ...
  it("proves src/source/education/corpus.ts is pure and browser-safe without Node imports", () => {
  ...
  it("refuses a corpus that presents itself as the national universe", () => {
    const corpus = loadEducationCorpusFromFile();

    expect(corpus.completeness.isNationalUniverse).toBe(false);
    expect(corpus.corpusScope).toBe(
      "sample-2022-vintage-not-national-universe",
    );
    ...
    const relabelled = {
      ...corpus,
      completeness: { ...corpus.completeness, isNationalUniverse: true },
    } as unknown as EducationCorpusSnapshot;
    expect(() => validateEducationCorpus(relabelled)).toThrow(
      /isNationalUniverse === false/,
    );

    const undisclosed: Record<string, unknown> = { ...corpus };
    delete undisclosed.completeness;
    expect(() =>
      validateEducationCorpus(
        undisclosed as unknown as EducationCorpusSnapshot,
      ),
    ).toThrow(/isNationalUniverse === false/);
  });

  it("rejects raw artifacts that do not satisfy the shared provenance contract", () => {
    const corpus = loadEducationCorpusFromFile();
    const [first] = corpus.rawArtifacts;
    expect(first).toBeDefined();

    const hashless = {
      ...corpus,
      rawArtifacts: [{ ...first!, sha256: "not-a-hash" }],
    } as unknown as EducationCorpusSnapshot;
    expect(() => validateEducationCorpus(hashless)).toThrow(/sha256/);
  });

  it("refuses education source data that carries attendance facts about people", () => {
    const corpus = loadEducationCorpusFromFile();

    // The shipped corpus is clean.
    expect(() => assertNoAutomaticAttendance(corpus)).not.toThrow();

    const contaminated = {
      ...corpus,
      institutions: [
        { ...corpus.institutions[0]!, enrolledPersonIds: ["person-1"] },
      ],
    } as unknown as EducationCorpusSnapshot;
    expect(() => assertNoAutomaticAttendance(contaminated)).toThrow(
      /attendance or enrollment facts/,
    );
  });

  it("refuses an organization conversion that guesses a jurisdiction", () => {
    const corpus = loadEducationCorpusFromFile();
    const school = findInstitutionByStableId("ipeds-unit:157085", corpus)!;
    const conversion = convertEducationInstitutionToOrganization(
      school,
      "1865-02-22",
    );

    expect(() => assertNoAutomaticAttendance(conversion)).not.toThrow();

    const guessed = {
      ...conversion,
      initialProfile: {
        ...conversion.initialProfile,
        locationJurisdictionId: "jurisdiction-fayette",
      },
    } as unknown as CreateOrganizationInput;
    expect(() => assertNoAutomaticAttendance(guessed)).toThrow(
