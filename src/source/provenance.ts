...
export interface EmpiricalRecordProvenance {
  ...
  /**
   * PROVENANCE-ASSESSMENT confidence only.
   * This must never be interpreted as statistical confidence, confidence interval,
   * MOE, RSE, model probability, or any empirical uncertainty statistic.
   */
  confidence?: string;
...
export function validateRawSourceArtifact(input: unknown): ValidationResult {
  ...
  // A full timestamp is preferred, but some retrievals only recorded the
  // calendar date. Accepting a date-only value is honest about that; widening
  // it to "T00:00:00Z" would invent a time of day the record never had.
  if (
    !isValidIsoTimestamp(input.retrievedAt) &&
    !isValidIsoDate(input.retrievedAt)
  ) {
    errors.push(
      "RawSourceArtifact.retrievedAt must be a valid ISO 8601 date or timestamp string representing actual retrieval time.",
    );
  }
  ...
  if (Object.hasOwn(input, "sourceReleaseDate")) {
    const val = input.sourceReleaseDate;
    if (val !== undefined && val !== null && !isValidIsoDate(val)) {
      errors.push(
        "RawSourceArtifact.sourceReleaseDate must be a valid ISO 8601 calendar date (YYYY-MM-DD) when present.",
      );
    }
  }

  if (Object.hasOwn(input, "documentationUrl")) {
    const val = input.documentationUrl;
    if (val !== undefined && val !== null) {
      if (!isNonEmptyString(val)) {
        errors.push(
          "RawSourceArtifact.documentationUrl must be a non-empty string when present.",
        );
      } else {
        try {
          new URL(val);
          ...
          errors.push(
            "RawSourceArtifact.documentationUrl must be a valid URL format.",
          );
        }
      }
    }
