...
export interface ProcedureDeclaration {
  ...
  /**
   * Each term names the literal words that fix it inside the excerpts. A
   * number must appear in its evidence as digits or a word; each list member
   * carries its own evidence phrase.
   */
  readonly terms?: Readonly<
    Record<
      string,
      | { readonly value: number; readonly evidence: string }
      | {
 ...
 */
export const PERSONNEL_PROCEDURE_DECLARATIONS: readonly ProcedureDeclaration[] =
  [
    {
      key: "mn-just-cause",
      ...
      terms: {
        grounds: {
          ...
          evidence: [
            ...
            "substandard performance",
            "insubordination",
            "serious violation of written policies and procedures",
          ],
        },
      },
    },
...
/** The number itself, as digits or its word, must be in the evidence. */
function numberSpelledIn(value: number, evidence: string): boolean {
  const word = NUMBER_WORDS[value];
  return (
    new RegExp(`(^|\\D)${value}(\\D|$)`).test(evidence) ||
    (word !== undefined && new RegExp(`\\b${word}\\b`).test(evidence))
  );
}
...
export function compilePersonnelProcedures(
  ...
  lock: ArtifactLock,
  declarations: readonly ProcedureDeclaration[] = PERSONNEL_PROCEDURE_DECLARATIONS,
  ...
  return declarations.map((declaration) => {
    ...
    const fixed = (evidence: string) =>
      declaration.excerpts.some((excerpt) => excerpt.includes(evidence));
    for (const [name, term] of Object.entries(declaration.terms ?? {})) {
      const bound =
        typeof term.value === "number"
          ? typeof term.evidence === "string" &&
            fixed(term.evidence) &&
            numberSpelledIn(term.value, term.evidence)
          : Array.isArray(term.evidence) &&
            term.evidence.length === term.value.length &&
            term.evidence.every((phrase) => fixed(phrase));
      if (!bound)
        throw new Error(
