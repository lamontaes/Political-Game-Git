...
import type { ResearchValidationEpisode } from "./types.js";
...
/**
 * Invented documents live apart from real ones and are reachable only by this
 * explicit path. The production compiler refuses this directory outright.
 */
const SYNTHETIC_DIR = path.resolve(
  process.cwd(),
  "data/source/legislative-state/__synthetic_fixtures__",
);
const OPENSTATES_DIR = path.join(SOURCES_DIR, "openstates");
const SYNTHETIC_OPENSTATES_DIR = path.join(SYNTHETIC_DIR, "openstates");
const RESEARCH_DIR = path.join(SYNTHETIC_DIR, "research_validation");
...
describe("National Legislative Source Corpus Compiler", () => {
  ...
  it("distinguishes explicit defeat from unresolved session adjournment", () => {
    ...
    const failedRaw = JSON.parse(
      fs.readFileSync(
        path.join(SYNTHETIC_OPENSTATES_DIR, "failed_bill_fixture.json"),
        "utf-8",
      ),
    );
    ...
    const unresolvedRaw = JSON.parse(
      fs.readFileSync(
        path.join(
          SYNTHETIC_OPENSTATES_DIR,
          "session_ended_unresolved_fixture.json",
        ),
        "utf-8",
      ),
    );
  ...
  it("preserves pure boundary: src/simulation remains untouched and decoupled", () => {
    ...
    const compilerTs = fs.readFileSync(
      path.resolve(process.cwd(), "src/source/legislative-state/compiler.ts"),
      "utf-8",
    );
