 ...
 *
 * SURFACE KINDS LIVE IN THE SCENE SPEC, NOT HERE. This module was written
 * against a second, finer vocabulary of its own, which is exactly the failure
 * it warns about one level up: a slot could be valid in the spec and unknown to
 * the binder, or legible under one floor and illegible under the other. The
 * distinctions that vocabulary drew and the spec's did not were folded into
 * `SCENE_SURFACE_KINDS`; the rest are recorded below as retired aliases so an
 * older authoring record can still be read and translated once.
 *
...
import {
  LEGIBILITY_REFERENCE_VIEWPORT_HEIGHT,
  MINIMUM_DYNAMIC_SURFACE_HEIGHT_PERCENT,
  MINIMUM_DYNAMIC_SURFACE_WIDTH_PERCENT,
  NON_COMPONENT_SURFACE_KINDS,
  isSceneSurfaceKind,
  minimumLegibleHeightPercent,
  SCENE_SURFACE_KINDS,
  type PercentRect,
  type SceneSurfaceKind,
...
/**
 * The finer surface names this module used before the convergence, mapped onto
 * the scene spec's single vocabulary.
 *
 * Kept as a translation table, not as a second set of kinds: nothing in this
 * repository authors against these any more, and `translateDonorSurfaceKind`
 * exists so an authoring record written before the fold can be read once and
 * rewritten. Six of the fifteen carried a distinction the spec could not
 * express and were added to `SCENE_SURFACE_KINDS` instead of being aliased —
 * they map to themselves here.
 */
export const DONOR_SURFACE_KIND_ALIASES: Readonly<
  Record<string, SceneSurfaceKind>
> = {
  "monitor-or-screen": "monitor-or-screen",
  "large-wall-map": "large-wall-map",
  "roll-call-board": "roll-call-board",
  "monitor-or-bulletin-board": "monitor-or-bulletin-board",
  "flag-standard": "civic-symbol",
  "court-seal-placard": "civic-symbol",
  "desk-document": "desk-document",
  "podium-placard": "podium-placard",
  "podium-speech-notes": "podium-speech-notes",
  "agenda-board": "agenda-board",
  "large-framed-chart": "large-framed-chart",
  "official-portrait-slot": "official-portrait-slot",
  "office-nameplate": "office-nameplate",
  "title-banner-safe": "title-banner-safe",
  "window-view": "window-view",
};

/**
 * Reads a surface kind written in either vocabulary. Returns `undefined` for a
 * name that belongs to neither, which is an authoring error rather than a
 * translation gap.
 */
export function translateDonorSurfaceKind(
  value: string,
): SceneSurfaceKind | undefined {
  if (isSceneSurfaceKind(value)) return value;
  return DONOR_SURFACE_KIND_ALIASES[value];
}
...
/**
 * The legibility floor lives in `environment-scene-spec.ts`, beside the surface
 * kinds it is applied to, so the spec validator and this binder cannot drift
 * apart. Re-exported here because this is where an author looks for it.
 */
export {
  FORESHORTENED_SURFACE_KINDS,
  FORESHORTENED_MINIMUM_HEIGHT_PERCENT,
  LEGIBILITY_REFERENCE_VIEWPORT_HEIGHT,
  NON_COMPONENT_SURFACE_KINDS,
} from "../environment/environment-scene-spec";

/** The unforeshortened height floor, as a share of plate height. */
export const MINIMUM_LEGIBLE_HEIGHT_PERCENT =
  MINIMUM_DYNAMIC_SURFACE_HEIGHT_PERCENT;
 ...
 */
export const COMPONENT_MINIMUM_WIDTH_PERCENT =
  MINIMUM_DYNAMIC_SURFACE_WIDTH_PERCENT;
...
export function slotIsPromotable(
  ...
  if (
    !(NON_COMPONENT_SURFACE_KINDS as readonly string[]).includes(kind) &&
...
export interface DynamicComponentFamily {
  ...
  readonly surfaceKinds: readonly SceneSurfaceKind[];
...
export const DYNAMIC_COMPONENT_FAMILIES: readonly DynamicComponentFamily[] = [
  ...
  },
  {
    ...
    surfaceKinds: ["roll-call-board", "monitor-or-screen", "agenda-board"],
  ...
  },
  {
    ...
    surfaceKinds: ["desk-document", "podium-speech-notes", "monitor-or-screen"],
...
export interface SlotComponentBinding {
  ...
  readonly surfaceKind: SceneSurfaceKind;
...
export function validateSlotComponentBindings(
  ...
  for (const binding of bindings) {
    ...
    if (!isSceneSurfaceKind(binding.surfaceKind)) {
      findings.push(
        bindingFinding(
          "unknown-surface-kind",
          "error",
          binding.slotId,
          `Surface kind '${binding.surfaceKind}' is not one of ${SCENE_SURFACE_KINDS.join(", ")}.`,
        ),
      );
      continue;
    }
