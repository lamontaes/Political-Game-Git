...
/*
 * SURFACE KINDS: these records were authored against the finer vocabulary the
 * convergence retired. `monitor-display` reads as `monitor-or-screen` here,
 * `district-map` as `large-wall-map`, and the flag and seal placards as the
 * scene spec's single `civic-symbol` kind; the six names that carried a
 * distinction the spec could not express were folded into SCENE_SURFACE_KINDS
 * and are unchanged. Nothing was re-measured — only renamed once, at the fold.
 */
...
interface ApartmentInput {
  ...
  readonly slots: readonly {
    ...
    readonly kind: SceneSurfaceKind;
    readonly zOrder: number;
    readonly rect: {
      readonly x_percent: number;
      readonly y_percent: number;
    ...
    readonly contentClasses: readonly SceneSurfaceContentClass[];
