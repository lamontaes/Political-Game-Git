...
describe("what the simulation owns in each room", () => {
  ...
  it("refuses every micro-frame it declared as decor, by the same rule", () => {
    ...
    // Probed as `large-framed-chart`, the framed surface that CAN host a
    // component. `picture-frame` is image-only and earns the lower bar a known
    // picture deserves, so asking the rule about it would answer a different
    // question than the one this test is asking: if somebody tried to promote
    // this frame to a chart, would the rule refuse it?
    const tooSmall = framed.filter(
      ({ decor }) =>
        !slotIsPromotable(decor.regionPercent!, "large-framed-chart")
          .promotable,
    );
...
describe("what may be drawn on the surfaces it owns", () => {
  ...
  it("binds no component at all to a window, a flag or a portrait", () => {
    const imageOnly = Object.values(PRODUCTION_SLOT_COMPONENT_BINDINGS)
      .flat()
      .filter((binding) =>
        ["window-view", "civic-symbol", "official-portrait-slot"].includes(
          binding.surfaceKind,
        ),
      );
