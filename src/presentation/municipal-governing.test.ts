...
describe("A39 municipal rule reception", () => {
  it("counts members voting separately from presence and rejects a duplicated person's ballot", () => {
    const { world, government, people } = charlottesvilleWorld();
    const seated = seatCouncil(world, government.key, people);
    const members = municipalSeats(seated, government.key).map(
      (seat) => seat.personId,
    );
    const dispositions = councilRoll(members, 2).map((entry, index) =>
      index === 2
        ? { ...entry, disposition: "present-not-voting" as const }
        : entry,
    );
    const result = evaluateMunicipalManagerElection(seated, {
      governmentKey: government.key,
      dispositions,
    });
    expect(result.ok).toBe(true);
    if (!result.ok) throw Error(result.reason);
    expect(result.presentMembers).toBe(3);
    expect(result.requiredYeas).toBe(2);
    const duplicate = evaluateMunicipalManagerElection(seated, {
      governmentKey: government.key,
      dispositions: [
        ...dispositions,
        { ...dispositions[0]!, memberKey: "another-key" },
      ],
    });
    expect(duplicate.ok).toBe(false);
    expect(serializeWorld(seated)).toBe(
      serializeWorld(deserializeWorld(serializeWorld(seated))),
    );
  });
});
