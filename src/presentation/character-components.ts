 ...
 * aside as soon as a RELEASED production candidate of that kind exists at or
...
export function resolveCharacterRecipe(
  ...
  if (bodyCandidates.length > 0) {
    ...
    for (const slot of library.slots) {
      ...
      // Body/head compatibility only. Pose and facing are applied in the two
      // stages below on purpose: narrowing by facing here would collapse "this
      // family has no art for the pose" and "it has the pose but not the
      // facing" into one indistinguishable diagnostic.
