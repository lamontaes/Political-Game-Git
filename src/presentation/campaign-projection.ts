    ...
    afterword:
      state.status === "won"
        ? supportedLegislativeTermDates(
          ...
          : `${candidateName} won. The seat is ${pronouns.possessivePronoun}, and so is everything that came before it.`
        : state.status === "lost"
          ? `${candidateName} lost. That is a thing that happened to ${pronouns.object}, not the end of ${pronouns.object}. Tomorrow is still there.`
          : null,
  };
}
