...
describe("Stage 6.5 visual integration contract", () => {
  ...
  it("composes approved released runtime visual assets deterministically for fixture people", () => {
    ...
    // Vertical placement is derived: the anchor's y IS the seat plane and the
    // recipe's root is the pelvis measured off the raster, so the figure sits
    // where the chair is rather than where a sprite was nudged to.
