...
export function selectPreparedBody(
  ...
  const features = appearance.material?.features;
  // A painted feature cannot silently become the destination's default.
  // Empty variants are the declared fixed artwork, not a missing selection.
  if (
    features &&
    Object.values(features).some(
      (value) => value.variant && !translate(value.variant, "face-feature"),
    )
  )
    return undefined;
  const material =
