...
it("refuses a profiled body whose successor kit omits compatible hair", () => {
  const f = fixture();
  const missingHair = f.added.filter((r) => r.component.kind === "hair-front");
  expect(missingHair.length).toBeGreaterThan(0);
  const records = f.records.filter(
    (r) => !missingHair.some((h) => h.asset_id === r.asset_id),
  );
  // Re-sign the declared partial generation so this reaches body kind coverage,
  // not the independent catalog-signature guard or orphan-part branch.
  const added = records.filter((r) => r.asset_id.endsWith("-prepared"));
  const generations = f.catalog.generations.map((g) =>
    g.generation === 2
      ? {
          ...g,
          component_ids: added.map((r) => r.asset_id).sort(),
  ...
  expect(() =>
    createCharacterComponentLibrary(records, { ...f.catalog, generations }),
  ).toThrow(/Incomplete prepared kit:.*has no compatible 'hair-front'/);
});
