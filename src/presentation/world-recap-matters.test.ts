...
describe("recap over W3's public matters", () => {
  ...
  it("over ordinary weeks, shows one entry per changed matter with its latest published stage and a count of its stages", () => {
    const frontier = life.world.history.nextSequence;
  ...
  it("orders by importance first, then by how recently the matter changed", () => {
    const frontier = life.world.history.nextSequence;
    const later = passWeeks(life.world, 26);
    const recap = projectWorldRecap(later, player, frontier, 50)!;
