...
)("R1 body change never substitutes face/hair", () => {
  for (const [g, pinned] of [
    ...
    it(`generation ${g} pinned=${pinned}`, { timeout: 120000 }, () => {
      ...
      const swapped = Object.entries(r.out)
        ...
        .reduce((n, [, v]) => n + v, 0);
      expect(
        Object.values(r.out).reduce((n, count) => n + count, 0),
      ).toBeGreaterThan(0);
      if (pinned)
        expect(
          Object.entries(r.out)
            .filter(([key]) => key !== "source-unresolvable")
            .reduce((n, [, count]) => n + count, 0),
        ).toBeGreaterThan(0);
      else
        // These later-generation selections have no pin. The legacy generation
        // cannot resolve them and must refuse, not silently upgrade the person.
        expect(Object.keys(r.out)).toEqual(["source-unresolvable"]);
