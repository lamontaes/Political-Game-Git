...
export class LegiScanAdapter implements LegislativeProviderAdapter {
  ...
  normalizeJurisdiction(
    ...
    if (!rawName) {
      throw new Error("LegiScanAdapter: Jurisdiction payload missing state name or abbreviation.");
    }

    const sourceKey = normalizeJurisdictionKey(data.state_abbr || data.state_name || "");
    ...
    return {
      sourceKey,
      provider: "legiscan",
      providerJurisdictionId: String(data.state_id || data.state_abbr || sourceKey),
  ...
  normalizeSession(
    ...
    const rawTag = data.session_tag || data.session_name || (data.year_start ? `${data.year_start} Regular Session` : null);
    if (!rawTag && !data.session_id) {
      throw new Error("LegiScanAdapter: Session payload missing session tag or ID.");
    }
    ...
    const classification: SessionClassification = data.special === 1 ? "special" : "regular";
  ...
  normalizeMeasure(
    ...
    const jurisdictionKey = normalizeJurisdictionKey(options.jurisdictionKey || data.state || "us_unknown");
    const sessionTag = options.sessionIdentifier || data.session?.session_tag || data.session?.session_name || String(data.session_id || "current");
    const sessionId = buildSessionId(jurisdictionKey, sessionTag);
    const measureId = buildMeasureId(jurisdictionKey, sessionTag, data.bill_number);

    const officialUrl = data.state_link || null;
    const providerUrl = data.url || (data.bill_id ? `https://legiscan.com/bill/${data.bill_id}` : null);
    ...
    // Text Versions
    const textVersions: LegislativeTextVersionSourceRecord[] = (data.texts || []).map((t, idx) => {
      const docLabel = t.type || `Version ${idx + 1}`;
      const textVersionId = buildTextVersionId(measureId, String(t.doc_id || docLabel));
      return {
        textVersionId,
        measureId,
    ...
    const actions: LegislativeActionSourceRecord[] = (data.history || []).map((h, idx) => {
      ...
      if (jurisdictionKey === "us_ne") {
        ...
        actingBody = "lower";
      } else if (chamberChar === "E" || (h.action || "").toLowerCase().includes("governor")) {
      ...
      if (lowerDesc.includes("signed by governor") || lowerDesc.includes("chaptered") || lowerDesc.includes("acts chapter")) {
        classifications.push("became-law", "executive-signature");
      }
      if (lowerDesc.includes("vetoed")) {
        classifications.push("executive-veto");
      }
      if (lowerDesc.includes("failed") || lowerDesc.includes("defeated") || lowerDesc.includes("tabled")) {
      ...
      return {
        ...
        actionDate,
        actingBody,
        actingBodyName: chamberChar === "S" ? "Senate" : chamberChar === "H" ? "House" : null,
    ...
    // Votes
    const votes: LegislativeVoteSourceRecord[] = (data.votes || []).map((v, idx) => {
      ...
      const rollCall: RollCallEntry[] = (v.roll_call || []).map((rc) => {
        ...
        if (text === "yea" || text === "yes" || rc.vote_id === 1) option = "yes";
        else if (text === "nay" || text === "no" || rc.vote_id === 2) option = "no";
    ...
    // Sponsors
    const sponsors: LegislativeSponsorSourceRecord[] = (data.sponsors || []).map((s, idx) => {
      const sponsorId = buildSponsorId(measureId, s.name || `sp_${s.people_id || idx}`, idx);
      const isPrimary = s.sponsor_type_id === 1 || (s.sponsor_order ?? idx) === 1;
      const sponsorshipType: SponsorshipType = isPrimary ? "primary" : "cosponsor";
    ...
    const measure: LegislativeMeasureSourceRecord = {
      ...
      officialUrl,
      providerUrl,
      rawProviderStatus: data.status !== undefined ? `status_${data.status}` : null,
      derivedLifecycle,
      subjects: [],
