...
export function assessOfficeQualifications(
  ...
  for (const row of rows) {
    ...
    if (row.field === "STATE_RESIDENCE" || row.field === "DISTRICT_RESIDENCE") {
      ...
      if (required === null || held === null) {
        assessments.push({
          ...
          reason:
            held === null
              ? `This office requires ${String(row.value)} of residence. The game has not recorded when this character came to live here, so it will not guess whether they qualify.`
              : `The residence requirement for this office is not a number of years the game can compare against, so it will not decide on residence.`,
          source: row,
        });
        continue;
      }
      assessments.push({
        ...
        reason:
          held >= required
            ? `Resident long enough: this office requires ${required} year${required === 1 ? "" : "s"} of residence.`
            : `Not resident long enough: this office requires ${required} year${required === 1 ? "" : "s"} of residence, and this character has lived here ${held}.`,
        source: row,
      });
      continue;
    }
    ...
    assessments.push({
      field: row.field,
      verdict: "not-evaluated",
      reason: `This office requires ${String(row.value)}. The game does not record that about a character, so it neither grants nor refuses on it.`,
      source: row,
    });
  }
