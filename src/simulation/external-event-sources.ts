...
export interface SourceTemporalCoverage {
  ...
  readonly updateCadence:
    "realtime" | "daily" | "monthly" | "annual" | "historical_archive";
}
...
export const AUTHORITATIVE_PROVIDER_REGISTRY: Readonly<
  Record<AuthoritativeProviderId, AuthoritativeProviderDefinition>
> = {
  noaa_ncei_storm_events: {
  ...
  fema_disaster_declarations: {
    ...
    description:
      "Official federally declared disasters, emergency declarations, and fire management assistance declarations since 1953.",
    defaultLicense: "US Government Public Domain Work",
    supportedFamilies: [
      "hurricane_tropical_storm",
      "tornado_severe_weather",
  ...
  doe_417_electric_emergency: {
    ...
    authoritativeOrganization:
      "U.S. Department of Energy (DOE) Office of Cybersecurity, Energy Security, and Emergency Response (CESER)",
  ...
  nifc_wildfire_data: {
    ...
    authoritativeOrganization:
      "National Interagency Fire Center (NIFC) / WFIGS",
  ...
  cdc_public_health_emergency: {
    ...
    description:
      "Authoritative national public health surveillance data, epidemic tracking, and public health emergency declarations.",
    defaultLicense: "US Government Public Domain Work",
  ...
  [
    {
      ...
      geographicEligibilityRules: [
        {
          ...
          description:
            "Hurricanes and tropical storms require coastal or direct inland tropical system path exposure.",
      ...
      calibration: {
        ...
        rationale:
          "Requires regional historical track frequency analysis per coastal segment before assigning annual sampling rates.",
      },
      knownLimitations: [
    ...
    },
    {
      ...
      calibration: {
        status: "unresolved_requires_research",
        rationale:
          "Tornado touchdown probability requires county-level grid frequency density maps (Tornado Alley vs East/West).",
      },
      knownLimitations: [
    ...
    },
    {
      ...
      calibration: {
        status: "unresolved_requires_research",
        rationale:
          "Requires regional NERC reliability entity failure distribution modeling.",
      },
      knownLimitations: [
    ...
    },
    {
      ...
      geographicEligibilityRules: [
        {
          ...
          description:
            "Earthquakes of magnitude M >= 5.0 are predominantly constrained to seismic hazard zones.",
          requiredStateCodes: [
            "AK",
            "CA",
            "HI",
      ...
      calibration: {
        ...
        empiricalBasis:
          "USGS National Seismic Hazard Model (NSHM) fault slip rates and historical seismicity catalog.",
    ...
    },
    {
      ...
      calibration: {
        status: "unresolved_requires_research",
        rationale:
          "Requires fuel load, drought index (KBDI), and ignitions frequency integration.",
      },
      knownLimitations: [
    ...
    },
    {
      ...
      calibration: {
        status: "unresolved_requires_research",
        rationale:
          "Requires epidemiological compartmental modeling (SIR/SEIR parameters).",
      },
      knownLimitations: [
...
export function validateExternalEventSourceContract(
  ...
  if (!contract.contractId || contract.contractId.trim().length === 0) {
    throw new Error(
      "External event source contract must have a non-empty contractId.",
    );
  }
  if (
    !contract.authoritativeProvider ||
    !contract.authoritativeProvider.providerId
  ) {
    throw new Error(
      "External event source contract must specify an authoritative provider.",
    );
  }
  const provider =
    AUTHORITATIVE_PROVIDER_REGISTRY[contract.authoritativeProvider.providerId];
  if (!provider) {
    throw new Error(
      `Unknown authoritative provider ID: ${contract.authoritativeProvider.providerId}`,
    );
  }
  ...
  if (contract.temporalCoverage.endIsoDate) {
    ...
    if (
      contract.temporalCoverage.endIsoDate <
      contract.temporalCoverage.startIsoDate
    ) {
      throw new Error(
        "Temporal coverage startIsoDate cannot be later than endIsoDate.",
      );
    }
  }
  ...
  if (contract.calibration.status === "calibrated") {
    if (
      ...
      throw new Error(
        "Calibrated event source contract must specify an explicit derivationFormula.",
      );
    }
    if (contract.calibration.samplePeriodYears <= 0) {
      throw new Error(
        "Calibrated samplePeriodYears must be greater than zero.",
      );
    }
  } else if (contract.calibration.status === "unresolved_requires_research") {
    if (
      !contract.calibration.rationale ||
      contract.calibration.rationale.trim().length === 0
    ) {
      throw new Error(
        "Unresolved calibration status must provide an explicit research rationale.",
      );
    }
  } else {
  ...
  for (const m of contract.seasonalApplicability.applicableMonths) {
    if (!Number.isInteger(m) || m < 1 || m > 12) {
      throw new Error(
        `Applicable month must be an integer between 1 and 12, got ${m}.`,
      );
    }
  }
...
export function isJurisdictionEligibleForEventFamily(
  ...
  if (
    ...
    !supportedStates.includes(stateCode.toUpperCase())
  ) {
    return false;
  }
  ...
  for (const rule of contract.geographicEligibilityRules) {
    if (
      rule.requiredStateCodes &&
      !rule.requiredStateCodes.includes(stateCode.toUpperCase())
    ) {
      return false;
    }
    if (
      rule.requiredCoastalAccess &&
      !COASTAL_US_STATE_CODES.has(stateCode.toUpperCase())
    ) {
      return false;
    }
  }
