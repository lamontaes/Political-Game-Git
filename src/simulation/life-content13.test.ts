...
import { chooseStoryOption, type StoryScene } from "../presentation/life-story";
import { sceneVenueForLocationKey } from "../presentation/scene-venues";
import {
...
describe("a favour is leaned on only after the covered shift was worked", () => {
  it("books the accepted favour into the existing calendar and performs it through Places", () => {
    ...
    const scene: StoryScene = {
      ...
      withPeople: asked.bindings.map((binding) => binding.personName),
      presentPeople: [],
      beat: asked,
    };

    const agreed = chooseStoryOption(world, {
      personId: fixture.playerId,
      scene,
      optionKey: "cover-it",
    });
    const shift = coveredShift(agreed, fixture.playerId)!;
    expect(shift).toBeDefined();
    expect(scheduledActivityState(agreed, shift.id).status).toBe("scheduled");
