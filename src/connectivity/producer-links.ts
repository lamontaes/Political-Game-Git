...
export function renderProducerLinks(
  ...
  const lines: string[] = [
    "# Who writes what, and who reads it",
    "",
    "Every producer this game runs, what it writes, what reads it, and what should read it and does not. A missing link is a defect, and names the thread that owns each end. Generated by `npm run connectivity:links -- render --write` from `docs/connectivity/links/`; do not edit by hand.",
    "",
    `Generated ${header.generatedAt} from ${header.head}. ${sorted.length} producers, ${allMissing.length} missing links: ${counts.join(", ")}.`,
    "",
