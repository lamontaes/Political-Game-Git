...
export function buildBriefingCard(
  ...
  const rateObs = relevantObs.find(
    (o: LausObservation) => o.measureCode === "03",
  );
  const lfObs = relevantObs.find(
    (o: LausObservation) => o.measureCode === "06",
  );
  const empObs = relevantObs.find(
    (o: LausObservation) => o.measureCode === "05",
  );
  const unempObs = relevantObs.find(
    (o: LausObservation) => o.measureCode === "04",
  );

  const seasonalText =
    seasonal === "S" ? "Seasonally Adjusted" : "Not Seasonally Adjusted";
  const periodLabel = rateObs
    ? `${rateObs.periodName} ${year}`
    : `${period} ${year}`;
  ...
  const reconciliationNote = recon.isReconciled
    ...
    : recon.discrepancyNote ||
      "Reconciliation check flagged rounding or data variations.";
