...
test("LIFE-PATHS2 pointer, keyboard, session, interruption and reload proof", async ({
  ...
  await page
    ...
    .click();
  await expect(page.getByRole("status")).toContainText("You enrolled");
  await page
    .getByRole("button", { name: "Schedule next session", exact: true })
    .focus();
  await page.keyboard.press("Enter");
  await expect(page.getByRole("status")).toContainText("calendar");
  await page
    .getByRole("button", {
  ...
  await page
    ...
    .click();
  await expect(page.getByRole("status")).toContainText("ended");
});
