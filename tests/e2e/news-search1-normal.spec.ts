...
test("normal News search stays usable on a narrow viewport after a second publication", async ({
  ...
  await search.tap();
  // The two generated headlines can share their first long word (for example
  // "filed"), so search the complete visible headline rather than assuming a
  // token is unique.
  await search.fill(firstHeadline);
