...
test("same-width new URL waits for decode, ignores stale URL and retains decoded paint after failure", async ({
  page,
}) => {
  await page.goto("/tests/e2e/support/raster-harness.html");
  const state = page.getByTestId("harness");
  const image = page.getByTestId("harness-image");
  await expect
    .poll(() => page.evaluate(() => window.rasterHarness.loads().length))
    .toBe(1);
  await page.evaluate(() => {
    window.rasterHarness.load(0);
    window.rasterHarness.decode(0);
  });
  await expect(image).toHaveAttribute("src", /#A$/);
  await page.evaluate(() => window.rasterHarness.identity("B"));
  await expect
    .poll(() => page.evaluate(() => window.rasterHarness.loads().length))
    .toBe(2);
  await expect(image).toHaveAttribute("src", /#A$/);
  ...
  await page.evaluate(() => {
    window.rasterHarness.load(1);
    window.rasterHarness.identity("C");
  });
  await expect
    .poll(() => page.evaluate(() => window.rasterHarness.loads().length))
    .toBe(3);
  ...
  await expect(image).toHaveAttribute("src", /#C$/);
  await expect(state).toHaveAttribute("data-pending", "false");
  await page.evaluate(() => window.rasterHarness.rerender());
  await expect(state).toHaveAttribute("data-revision", "1");
  expect(await page.evaluate(() => window.rasterHarness.loads().length)).toBe(
  ...
  await page.evaluate(() => window.rasterHarness.identity("D"));
  await expect
    .poll(() => page.evaluate(() => window.rasterHarness.loads().length))
    .toBe(4);
  await page.evaluate(() => {
    window.rasterHarness.load(3);
    window.rasterHarness.reject(3);
  });
  await expect(image).toHaveAttribute("src", /#C$/);
  await expect(state).toHaveAttribute("data-pending", "true");
});
