...
test("normal prepared person changes materials/features and real clothes, then reopens", async ({
  ...
  await page
    .getByRole("combobox", { name: "Skin tone", exact: true })
    .selectOption("brown");
  await page.getByText("Facial features", { exact: true }).click();
  ...
  for (const name of ["Shirt style", "Trouser style"]) {
...
test("prepared room action menu Escape and Talk restore its real invoker", async ({
  page,
}) => {
  ...
  await page.keyboard.press("Enter");
  await expect(menu.locator("button:not([disabled])").first()).toBeFocused();
  await page.getByTestId("action-talk").focus();
  await page.keyboard.press("Enter");
  const talk = page.getByRole("region", { name: /^Conversation with / });
  await expect(talk).toBeVisible();
  await expect(
    talk.getByTestId("conversation-intents").getByRole("button").first(),
  ).toBeFocused();
  await page.keyboard.press("Escape");
