...
async function freshBrowser(page: Page) {
  await page.goto("/?seed=pt3-school-corridor");
...
test.describe("PT3 — the corridor scene on the screen", () => {
  test("names what got broken and the classmate who broke it", async ({
    page,
  }) => {
    await freshBrowser(page);
    await walkCreator(page, { age: 10, childhood: true });
    ...
    await page
      ...
      .getByRole("button", { name: new RegExp(`^Say it was ${classmate}`) })
      .click();
