...
for (const viewport of [
  ...
  test(`apartment calibration on ordinary controls at ${viewport.width}`, async ({
    page,
  }, info) => {
    await page.setViewportSize(viewport);
    await page.goto("/?seed=people-fresh-1&art-preview=candidate");
    ...
    expect(bounds!.y + bounds!.height).toBeLessThan(viewport.height - 40);
    expect(bounds!.height / viewport.height).toBeLessThan(0.55);
    ...
    for (const layer of geometry) {
      ...
      expect(layer.bottom).toBeLessThan(viewport.height - 40);
    }
    // This scene's foreground contact is y92%; the unchanged covering camera
    // maps it to y92% at these two aspect ratios, once for room and figure.
    expect(
      Math.abs(bounds!.y + bounds!.height - viewport.height * 0.92),
    ).toBeLessThan(2);
    const records = await page.getByTestId("scene-backdrop").evaluate((el) => ({
...
test("apartment furniture silhouette masks the rear depth control only", async ({
  page,
}, info) => {
  await page.goto("/tests/e2e/support/scene-depth.html?scene=apartment");
