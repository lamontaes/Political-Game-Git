...
/*
 * Whether this checkout has any candidate-review material at all.
 *
 * The review bank is private. A public checkout is forbidden to carry it, and
 * with the retired Visual4 cast deleted there is nothing left under
 * `art/generated/candidates/` for the review catalog to assemble from, so it
 * reports catalog generation 0 and an empty component map. That is an absence
 * of material, not a broken preview, and the two must not share an outcome:
 * read as a failure it threw out of a `describe` body and took all thirty-nine
 * assertions in this file down at collection time, before one of them ran.
 *
 * Where the bank IS present — the owner's Mac — every suite below runs and
 * must pass exactly as before. Where it is absent they are skipped, and the
 * invariant immediately below still runs, so a half-loaded or genuinely broken
 * bank cannot hide behind the skip.
 */
const REVIEW_BANK_PRESENT = PREVIEW.characters.catalogGeneration >= 1;
const NO_REVIEW_BANK =
  "no candidate-review bank on this checkout: the character catalog reports no generation and no components, which is the ordinary state of a checkout that may not carry the private pack";

describe("the candidate-review bank is present, or absent for a stated reason", () => {
  it("never reports a generation it has no components for, or the reverse", () => {
    if (!REVIEW_BANK_PRESENT) console.info(`art-preview: ${NO_REVIEW_BANK}`);
    expect(PREVIEW.characters.components.size > 0).toBe(REVIEW_BANK_PRESENT);
  });
});
...
describe.runIf(REVIEW_BANK_PRESENT)(
  ...
  () => {
    ...
    it("is the same canonical people, not a cast invented for the preview", () => {
      expect(
        [...preview.map((person) => person.personId)].sort(),
      ).toStrictEqual([...production.map((person) => person.personId)].sort());
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "the preview does not put an adult body on a child",
  () => {
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "the fitted figure is scaled, not stretched",
  () => {
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "a previewed figure still reports what is wrong with it",
  () => {
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "the DEFAULT wardrobe resolver uses the catalog that draws",
  () => {
    /*
    ...
    /* Guarded because this runs at collection time, not inside a test: with no
     * review bank the resolver throws here and the whole file fails to load. */
    const candidateTop = REVIEW_BANK_PRESENT
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "a complete, drawn composition still carries its diagnostics",
  () => {
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "one catalog decides the whole of one person's picture",
  () => {
...
describe.runIf(REVIEW_BANK_PRESENT)(
  "clothes can be chosen out of the catalog that draws them",
  () => {
