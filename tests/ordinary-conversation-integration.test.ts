...
// On 2026-09-22 the owner removed the evening "sit and talk" invitation, and the
// opening day no longer writes its two records (the invitation and its hold),
// so the frontier this conversation starts from moved from 114 to 112.
// On 2026-09-23 the opening day stopped writing the reasonless Saturday
// afternoon invitation (its event, tentative hold, hold state and knowledge)
// and the picnic favor and confidence (an event and knowledge each): none came
// from the asker's own records. Eight records fewer moves the frontier from 112
// to 104; the wording is unchanged.
...
describe("PR79 optional consequence hook preserves ordinary subjects", () => {
  it("accounts explicitly for P2R2 initialization while preserving wording", () => {
    ...
    expect(current.household.initialNextSequence).toBe(104);
    expect(current.householdCallback.initialNextSequence).toBe(104);
  ...
  it("keeps every generated identity joined to the record it references", () => {
    ...
    for (const household of [replay.household, replay.householdCallback]) {
      ...
      for (const due of household.records.aftermath) {
        ...
        expect(due.stableKey).toContain("frontier-104");
      }
      for (const turn of household.records.turns) {
        ...
        expect(turn.semantic.turnKey).toContain("frontier-104");
      }
    }
  });
