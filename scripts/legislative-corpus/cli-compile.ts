...
const OUTPUT_DIR = path.resolve(process.cwd(), "data/legislative_source/corpus");
const MANIFESTS_DIR = path.resolve(process.cwd(), "data/legislative_source/manifests");

export function runCorpusCompilation(timestamp: string = "2026-08-28T00:00:00Z") {
  console.log("================================================================================");
  ...
  if (fs.existsSync(OPENSTATES_DIR)) {
    const files = fs.readdirSync(OPENSTATES_DIR).filter((f) => f.endsWith(".json"));
    console.log(`Ingesting ${files.length} Open States fixture(s) from ${OPENSTATES_DIR}...`);
  ...
  if (fs.existsSync(LEGISCAN_DIR)) {
    const files = fs.readdirSync(LEGISCAN_DIR).filter((f) => f.endsWith(".json"));
    console.log(`Ingesting ${files.length} LegiScan fixture(s) from ${LEGISCAN_DIR}...`);
    for (const file of files) {
  ...
  console.log("Compiling deterministic normalized legislative corpus package...");
  ...
  const manifestPath = path.join(MANIFESTS_DIR, "national_coverage_manifest.json");
  ...
  console.log(`  Jurisdictions registered: ${corpusPackage.manifest.totalJurisdictions}`);
  console.log(`  Sessions indexed:         ${corpusPackage.manifest.totalSessionsIndexed}`);
  console.log(`  Measures normalized:      ${corpusPackage.measures.length}`);
  console.log(`  Text versions indexed:    ${corpusPackage.textVersions.length}`);
  ...
  console.log(`  Package Checksum:         ${corpusPackage.buildMetadata.checksum}`);
  console.log(`  Manifest Checksum:        ${corpusPackage.manifest.sha256}`);
  console.log(`  Output corpus size:       ${(corpusSizeBytes / 1024).toFixed(2)} KB`);
  console.log(`  Manifest size:            ${(manifestSizeBytes / 1024).toFixed(2)} KB`);
  console.log("================================================================================");
