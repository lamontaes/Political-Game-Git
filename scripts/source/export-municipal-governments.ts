...
const REGISTRY_OUTPUT = "src/simulation/municipal-rule-registry.generated.ts";
...
export function compileMunicipalProjection() {
  ...
  return {
    governments: ordered as unknown as MunicipalGovernment[],
    meta: {
...
export function renderMunicipalGovernments(
  projection = compileMunicipalProjection(),
): string {
  const { governments: ordered, meta } = projection;
...
export const MUNICIPAL_GOVERNMENTS_META = ${JSON.stringify(meta, null, 2)} as const;
...
/** Precompute precisely the existing admission result, not a second rules engine. */
export function renderMunicipalRuleRegistry(
  projection = compileMunicipalProjection(),
): string {
  const packs = projection.governments.flatMap((government) => {
    const result = municipalRulePackFor(government);
    return result.ok && result.evidence === "enacted-text" ? [result.pack] : [];
  });
  return `/** GENERATED by export-municipal-governments.ts; do not edit. */
/** Complete admitted registry, not an unloaded inventory. See the full inventory for refusals. */
export const MUNICIPAL_RULE_REGISTRY_META = ${JSON.stringify(
    {
      productionCorpusSha256: projection.meta.productionCorpusSha256,
      kentuckyCorpusSha256: projection.meta.kentuckyCorpusSha256,
      nationalCorpusSha256: projection.meta.nationalCorpusSha256,
      referenceObservationSha256: projection.meta.referenceObservationSha256,
      inventoryGovernments: projection.governments.length,
      admittedPacks: packs.length,
    },
    null,
    2,
  )} as const;

export const MUNICIPAL_RULE_PACKS_JSON: string = ${JSON.stringify(JSON.stringify(packs))};
`;
}

function main(): void {
  const projection = compileMunicipalProjection();
  const module = renderMunicipalGovernments(projection);
  const registry = renderMunicipalRuleRegistry(projection);
  if (process.argv.includes("--check")) {
    ...
    const registryCheckIndex = process.argv.indexOf("--check-registry-file");
    const registryTarget =
      registryCheckIndex >= 0
        ? process.argv[registryCheckIndex + 1]!
        : resolve(REPO_ROOT, REGISTRY_OUTPUT);
    if (readFileSync(registryTarget, "utf8") !== registry)
      throw new Error(
        "Municipal runtime registry differs from its locked sources. Run export:municipal-governments.",
      );
    process.stdout.write(
      "Municipal browser projection and runtime registry regenerate byte-identically.\n",
    );
    return;
  }
  writeFileSync(resolve(REPO_ROOT, OUTPUT), module, "utf-8");
  writeFileSync(resolve(REPO_ROOT, REGISTRY_OUTPUT), registry, "utf-8");
