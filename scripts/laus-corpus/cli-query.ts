...
export function runQuery(args: string[] = process.argv.slice(2)): void {
  if (!fs.existsSync(COMPILED_PATH)) {
    console.error(
      `Compiled corpus not found at ${COMPILED_PATH}. Run compile first.`,
    );
    process.exit(1);
  }

  const compiled: LausCompiledCorpus = JSON.parse(
    fs.readFileSync(COMPILED_PATH, "utf-8"),
  );
  ...
  for (const arg of args) {
    ...
    if (arg.startsWith("--seasonal="))
      filter.seasonal = arg.slice(11).toUpperCase() as "S" | "U";
  }

  const result = queryCorpus(compiled, filter);
  console.log(
    `Matched ${result.totalMatchedObservations} observations across ${result.areas.length} areas.`,
  );
  ...
  for (const area of result.areas.slice(0, 5)) {
    ...
    console.log(
      `  Unemployment Rate: ${card.unemploymentRateText} (${card.seasonalAdjustmentText})`,
    );
    console.log(
      `  Labor Force: ${card.laborForceText} | Employment: ${card.employmentText} | Unemployment: ${card.unemploymentText}`,
    );
    console.log(
      `  Status: ${card.statusText} | Reconciliation: ${card.reconciliationNote}`,
    );
  }
}
...
if (
  ...
  process.argv[1]?.includes("cli-query")
) {
  runQuery();
}
