...
describe("evidence reconciliation (P125-REPAIR-02 phase 3)", () => {
  ...
  it("reports counts that match a live measurement, not a stale run", () => {
    // Combined LAND13 measurement: EDU153 + ENV + FISCAL + CAREER-PATH7 +
    // DISTRICTS13, reconciled onto current main. Regenerated by
    // corpus:prose; these are the numbers it reports on this tree.
    const coverage = buildCoverageReport(inventory);
    expect(coverage.totalLiterals).toBe(67406);
