...
describe("evidence reconciliation (P125-REPAIR-02 phase 3)", () => {
  ...
  it("reports counts that match a live measurement, not a stale run", () => {
    ...
    // accepted main with the ENV-ALL1 scene/travel and PEOPLE-VISUAL4
    // candidate assembly modules together. The generator was re-run on the
    // merged tree; these are the numbers it reports.
    const coverage = buildCoverageReport(inventory);
    expect(coverage.totalLiterals).toBe(66210);
    expect(coverage.counts.INVENTORIED).toBe(2107);
    expect(coverage.scannedFiles).toBe(480);
  });
});
