...
describe("evidence reconciliation (P125-REPAIR-02 phase 3)", () => {
  ...
  it("reports counts that match a live measurement, not a stale run", () => {
    // Combined LAND13 measurement plus DESKTOP-CLIENT1 save-transfer copy.
    // Regenerated by corpus:prose; these are the numbers it reports on this tree.
    const coverage = buildCoverageReport(inventory);
    expect(coverage.totalLiterals).toBe(67607);
    expect(coverage.counts.INVENTORIED).toBe(2145);
    expect(coverage.scannedFiles).toBe(512);
  });
});
