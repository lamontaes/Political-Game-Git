...
describe("evidence reconciliation (P125-REPAIR-02 phase 3)", () => {
  ...
  it("reports counts that match a live measurement, not a stale run", () => {
    // Combined LAND13 measurement: current main (EDU-PATH7 + PEOPLE-VISUAL4)
    // plus the ENV-ALL1 scene/travel modules. Regenerated by corpus:prose;
    // these are the numbers it reports.
    const coverage = buildCoverageReport(inventory);
    expect(coverage.totalLiterals).toBe(66558);
    expect(coverage.counts.INVENTORIED).toBe(2107);
    expect(coverage.scannedFiles).toBe(489);
  });
});
