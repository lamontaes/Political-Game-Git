...
Four of them are **not committed**: \`coverage-candidates.json\`,
\`prose-inventory.json\`, \`prose-inventory.csv\` and \`review-packet.html\`.
They are the large ones, and committing a fresh copy of each on every prose
change had put about 3.4 GB of near-duplicate revisions into Git history —
more than half the repository — for files this generator rebuilds on demand.
Run \`npm run corpus:prose\` to produce them locally. The small
human-readable reports below stay tracked, and so does
\`metrics-baseline.json\`, because an accepted baseline is a record rather
than a derivative. \`check\` compares whichever of them is present on disk.
...
function main(): void {
  ...
  if (mode === "check") {
    ...
    for (const [name, expected] of exact) {
      ...
      compared += 1;
      if (found !== expected) drift.push(name);
    }
    const packetOnDisk = committed("review-packet.html");
    if (packetOnDisk !== null) {
      compared += 1;
      if (
    ...
    process.stdout.write(
      `corpus:check OK — ${inventory.counts.total} templates, ${diagnostics.warnings.length} warnings, ${coverage.counts.NEEDS_CLASSIFICATION} unclassified candidates. ${compared} of ${exact.length + 1} artifacts present and matching a fresh regeneration; review-packet.html, when present, is compared with the commit it records blanked.\n`,
    );
    return;
  }
