...
export function integrateProvenance(intakePath: string, outputDir: string) {
  ...
  const sheet13 = intake.find(
    (e: unknown) => (e as { sheet_number: number }).sheet_number === 13,
  );
  ...
  const sheetProv = {
    ...
    rights_license_status: sheet13.rights_status, // This is correctly 'unknown' now based on previous fixes
    reference_type: "measured-drawing",
    source_authority_category: "authoritative",
    approval_status: "pending", // Fixed: DO NOT mark research sources as 'approved' automatically
  };
  ...
  const derivedProv = {
    ...
    generation_edit_date: "2026-08-25T23:38:00.000Z", // Fixed: Use a stable deterministic date, NOT a volatile Date.now()
  };
