...
`GARMENT_FIT_DEFAULT_BOUNDS`, overridable per bank and validated on every
profile:
...
| `fit-profile-out-of-bounds`         | The authored profile fails the bank's own limits.                                                 |
| `fit-bands-unsupported-by-renderer` | The fit is a bounded warp and the caller did not declare band support.                            |
...
The judged number for a top or a bottom is the **proportional residual**: how
far the garment's edge sits from where it would sit if it met the target body in
the same proportion it meets the body it was drawn for. Comparing the garment's
edge directly to the body's is the obvious metric and it is wrong, because a
garment is not skin: a knit carries ease, and ease is not error. Measured against
the body's outline a perfectly fitted garment can never score zero.
...
| -------------------- | ------ | -------- | ------- | ------ | ----------------------- |
| `fit-knit-olive`     | lean   | 17.2 px  | 5.0 px  | 1.2 px | `affine-reusable`       |
| `fit-knit-olive`     | heavy  | 30.0 px  | 12.1 px | 4.0 px | `bounded-warp-reusable` |
| `fit-trousers-slate` | lean   | 13.6 px  | 1.4 px  | 0.9 px | `affine-reusable`       |
| `fit-trousers-slate` | heavy  | 22.7 px  | 2.6 px  | 1.4 px | `affine-reusable`       |
...
— 30.0 px to 12.1 px — and still misses: 12.1 px is 4.8% of the heavy
...
both. The bounded warp reaches 4.0 px (1.6%) with a widest-to-narrowest band
spread of 1.17x and a maximum seam step of 1.02x, well inside the limits.

That is the whole case for the escape hatch, and it is also its whole scope: the
warp exists for the one thing an affine measurably cannot do.
...
## Rendering a bounded warp

A warped layer resolves to 16 ordered bands, each an axis-aligned rectangle plus
the fraction of the source raster it reveals. `CharacterRenderLayer.bands`
carries them in plate percent units with `clipTopPercent` / `clipBottomPercent`
in the form CSS `inset()` wants: draw the same image once per band, positioned
and scaled for that band, clipped to its own slice. Nothing is resampled and no
pixel is written.

No renderer in this repository draws bands yet. A caller therefore declares
`rendererSupportsBands` on the render plan request; it defaults to false, and a
warped layer is refused rather than drawn as its bounding rectangle — which would
paint the garment at its widest band from top to bottom, a worse result than the
unfitted placement the fit replaced. No component in the runtime bank currently
resolves to a warp, so nothing is withheld today.
