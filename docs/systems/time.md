...
Stage 6 Run B aggregate evaluation is explicit and writes no automatic daily/monthly/quarterly tick. It adds no recurrence or scheduler. A later domain may schedule one named evaluation or follow-on through the existing due-item mechanism, but cannot introduce a parallel clock or opaque recurrence expression.
