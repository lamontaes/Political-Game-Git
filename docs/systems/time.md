...
Stage 6 Run A adds the one generic persistent future due-item mechanism and integrates it into `advanceWorld`; it does not add a second time API. A due identity contains its due date, open transition key, stable domain references, jurisdiction/provenance, and append-oriented state—not an opaque payload, closure, formula, recurrence rule, or copied domain record. Crossed scheduled items resolve by due date and then creation/global sequence through injected deterministic nonserialized handlers. The framework exposes the due date as valid handler chronology, requires exactly one resolved/blocked/cancelled terminal state, and optionally links an ordinary outcome event. At a committed current-time frontier, a latest scheduled state may never be due now or overdue: the advance settles each same-date batch before that frontier is validated. Terminal items never rerun.

Before a proposed advance commits, every currently crossed transition key must have a handler. Missing or failing handlers, input mutation, direct scheduler-state writes, and invalid outcome links reject the entire immutable result; the caller's world is unchanged. Worlds without pending due items retain prior behavior. Synthetic handlers prove the contract, while elections, policy phases/sunsets, terms, appointments, incidents, mortality, finance recurrence, and other domain transitions remain with their owning later systems.
