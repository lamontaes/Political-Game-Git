...
3. **Strict Subsystem Isolation**: Implementation is isolated in `src/source/government-universe/`, `scripts/source/government-universe/`, `data/source/government-universe/`, and `tests/government_universe.test.ts`. Current gameplay, `src/simulation/`, `src/player/`, and `src/presentation/` remain untouched.
...
Normalized data models in `src/source/government-universe/types.ts`:
...
The compiler produces 6 authoritative summary manifests in `data/source/government-universe/manifests/`:
