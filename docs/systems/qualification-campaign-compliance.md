...
Both compile as staged secondary research. `KNOWN`, `UNKNOWN`,
...
- Kentucky candidate campaign statement, reporting schedule, electronic
  ...
  rules from current KRS 121.180 and current Registry guidance.
...
`us-ak-legislature-v1:house` and `us-ak-legislature-v1:senate`. Every other
existing candidacy option retains the already-labelled game adult fallback and
unknown jurisdiction qualifications.
