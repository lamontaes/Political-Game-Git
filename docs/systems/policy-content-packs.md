...
A new world ships with an empty policy catalog: no domains, no issues, no
...
The catalog was emptied on purpose. Before `production-catalog.ts` existed
there was exactly one set of catalogs, built by `createSynthetic*Catalog` to
...
The obvious repair — author a starting catalog and relax the boundary — fails
...
composes the world's catalog from that registry. `POLICY_PACKS` ships empty,
...
The reason is the reason the catalog was emptied in the first place. From
...
_undeclared_: a definition in the world's catalog that no loaded pack accounts
...
metric, mechanism, incident and mortality catalogs are untouched and still
empty.
...
did not and why, rather than a shorter catalog than they expected.
...
when it cites documents, `authored-fiction` when it is a design judgment about
how something should feel. Those are different declarations and the difference
is the point. A design judgment laundered into `sourced` because it arrived in
