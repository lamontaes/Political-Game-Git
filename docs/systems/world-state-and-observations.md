...
World schema 12, generator `demo-world-v12`, metric catalog `world-metric-catalog-v2`, causal catalog `causal-mechanism-catalog-v1`, and snapshot format 11 preserve the metric/causal catalogs and every history family exactly through JSON and the Node-only SQLite repository. This includes policy alternatives, operations, baselines, implementation profiles, estimates, and realizations plus each activation's typed point-target or exact interval-total magnitude basis. Load integrity rejects missing, malformed, unavailable, or incompatible references. Person materializer version 4 remains unchanged. Unsupported older versions remain rejected; no fabricated migration sequence was added.

## Deferred systems

Run B owns the bounded causal/effect and aggregate economy/fiscal derivation layer described in [Causal Mechanisms, Lightweight Economy, and Fiscal Continuity](causal-economy-and-fiscal.md). Run C adds explicit quantitative policy projection and realization described in [Quantitative Policy](quantitative-policy.md). Runs A–C still add no automatic economy/fiscal tick, formula language, generalized incident prerequisites/selection/chains, natural-disaster or recession generator, mortality/incapacity/evidence system, law/institutions/elections/campaigns, media ecology, public opinion, territory-specific content, foreign-government simulation, or player-facing UI. Runs D–E and later stages own those boundaries.
