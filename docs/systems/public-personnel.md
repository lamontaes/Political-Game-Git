...
The current root registration is additive. `NewGameStartingLife` contains the
ordinary, legislative, judicial and state-agency tokens; the judicial and
state-agency tokens both reuse the ordinary-life questionnaire context before
their mutually exclusive post-build initializer runs. `PlayerGame` exposes the
state-agency choice only for a selected state with compiled personnel
procedures and an age at or above the start's minimum. Changing place clears
only a legislative or state-agency choice that the new state cannot support.

## Structured player evidence

The personnel panel is an adapter over canonical history, not an allegation
writer. An informal meeting requires selection of an existing dated event
about the employee that is tagged as an informal-resolution subject. A
disciplinary notice requires a named statutory cause and an existing event
tagged for that cause. In both cases the controlled authority must have direct
participation in, or accurate knowledge of, the event. Future, unrelated,
untagged and unknown events are excluded.

The selected record deterministically supplies the meeting note, specific
reasons and visible notice preview. Those strings continue through #178's
existing action payload and personnel record writers; there is no second
evidence, employment or personnel store. When no qualifying record exists, the
panel says the action is unavailable instead of inviting free-text invention.
