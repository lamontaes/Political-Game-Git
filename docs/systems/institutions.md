...
Stage 6 world-metric scopes, causal effect targets, and optional future due-item jurisdictions reuse the same stable open jurisdiction IDs. A metric definition may legitimately have no state or observation for a jurisdiction, and no coverage is fabricated. Run B aggregate revenue/outlays/debt are quantitative world state, not an institution, treasury, appropriation, tax authority, office, budget procedure, law, or account. Later effective law, governing producers, and sourced civic/statistical content remain separate dependencies; no territory-specific series or hard-coded state hierarchy exists.
