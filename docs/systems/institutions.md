...
Stage 6 world-metric scopes, causal effect targets, policy operations, and optional future due-item jurisdictions reuse the same stable open jurisdiction IDs. A metric definition may legitimately have no state, observation, policy operation, or forecast for a jurisdiction, and no coverage is fabricated. Run B aggregate revenue/outlays/debt are quantitative world state, not an institution, treasury, appropriation, tax authority, office, budget procedure, law, or account. Run C implementation factors are explicit scenario inputs and evidence, not inferred institutional authority or capacity truth. Later effective law, governing producers, and sourced civic/statistical content remain separate dependencies; no territory-specific series or hard-coded state hierarchy exists.
