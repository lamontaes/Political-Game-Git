...
Every Stage 5 Run C family uses deterministic stable identity, provenance, one contiguous global append sequence, chronology and dangling-reference validation, date-plus-sequence queries, and exact JSON/Node-only SQLite persistence. The current Stage 6 Run C boundary is world schema 12 and snapshot format 11; the Stage 5 identities and semantics are unchanged.
