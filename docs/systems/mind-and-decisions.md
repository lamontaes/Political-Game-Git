...
Historical evaluation uses a `HistoricalCutoff` containing both an as-of date and an exclusive global history sequence. Both must be satisfied. This prevents a later-appended, backdated history record from leaking into a decision that was evaluated earlier. Biography facts do not carry append-sequence availability, so the subjective builder and decision-source validator include them only at the world's current date and current end-of-history frontier; durable traces freeze any factual source content used there. Reconstructing an older cutoff cannot introduce a biography fact whose availability at that time is unprovable. Canonical Stage 5 sources—including resource, obligation, dwelling, occupancy, and tenure families—use a closed typed record-family reference; validation requires the record to exist, involve the actor directly or through historically available household membership where supported, and precede both cutoff dimensions.
...
Stage 6 Run C policy analysis follows the same subjective boundary. Recording a baseline or estimate grants nobody knowledge. An ordinary analysis-review event plus person-owned event knowledge makes one estimate available to one actor. The policy decision adapter accepts only that actor's knowledge and caller-supplied interpretation alongside existing values, goals, beliefs, relationships, and commitments; it does not map exact magnitude to ideology, political support, or a universal feasibility score.

World schema 12 and snapshot format 11 preserve the mind catalog, control state, all Stage 4 history families, complete Stage 5 life/resource/housing records, Stage 6 metric/observation/causal/effect/policy/due histories, generated provenance, typed life-history source references, and the complete decision trace through deterministic JSON and Node-only SQLite round trips. Unsupported older versions remain rejected until migrations exist.
...
The current foundation implements bounded formative/adult situation consequences, explicit meaningful relationship moments, evidence-backed resource pressure, and non-omniscient aggregate causal/economic truth, but not autonomous personality development, autonomous memory recall, automatic relationship upkeep/decay, autonomous finance/housing behavior, generalized incident selection/chains, institutions and mutable law, elections, campaigns, legislation, public-opinion populations, dialogue, or a polished player UI. Causal/economic records do not enter perception or decisions automatically; a person still requires an observation/ordinary knowledge path. Situations may append an appraisal or return a non-applying repeated-history development proposal; they do not silently create tendencies, values, goals, or beliefs.
