...
World schema 8 and snapshot format 7 preserve the mind catalog, control state, all Stage 4 history families, Runs A/B life records, generated provenance, typed life-history source references, and the complete decision trace through deterministic JSON and Node-only SQLite round trips. Unsupported older versions remain rejected until migrations exist.
