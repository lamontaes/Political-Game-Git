# P31-OPS: Play-first launch guidance and bounded failure triage

Owner: isolated OPS successor, Codex / GPT-5.6 Sol / High / Standard.
Workspace: `/Users/lamontae/.codex/worktrees/6949/Political Game`.
Branch: `codex/p31-ops-play-first`; starting main/source:
`68f661665ded130c0154edb408be2a11342b78c3`.
Authority: PATCH31 permanent owner requirement and P31-OPS in DELIVERY28,
current Assignment Board dispatch checked 2026-09-13. A alone integrates main.

## Narrow increment

The identified server prints ordinary Play first, candidate-art Play with
separate saves second, and developer review fixtures third. The existing
double-click launcher opens its selected ordinary gameplay route, preserving
its candidate default and production option, with explicit art-mode and review
labels. It retains exact source/dependency fingerprints, identity-checked opening,
loopback serving, occupied-port refusal and no validation-before-play.
No simulation input, time, money, save migration, art admission or legal behavior
changes. This does not complete failure triage, upgrades, gameplay or packaging.

Owned payload: `scripts/dev-identified.mjs`, `scripts/play-mac.command`,
`tests/dev-identified.test.ts`, `tests/play-mac.test.ts`, this plan and the
single patch release declaration. No C/root/menu/desktop changes.

## Validation and delivery

Use `agent:test-cadence`, scoped format/lint, shell syntax, typecheck and exact
release declaration range. Launcher CLI tests retain the existing five-second
assertion deadlines and lifecycle/port assertions, adding destination order,
seed encoding, loopback refusal and an actual shell-launcher fixture for both
art modes, cached/missing dependencies and occupied-port refusal. The shell
fixture uses only its own temporary directories and external-command stubs;
it is not browser/human acceptance or an owner play-copy modification.

Original failed receipts are preserved outside Git under
`/private/tmp/p31-ops-receipts`, `/private/tmp/p31-ops-launch-permitted` and
`/private/tmp/p31-ops-launch-final`. These include sandbox socket EPERM, fixture
quoting/early-exit errors and an unchanged five-second occupied-port assertion
timeout during shared-host compiler/unit contention. Do not relabel them green.
The first typecheck command succeeded but source edits during execution prevented
receipt certification. Recheck stable source before publication.

Publish the independent launcher fix on its normal branch/PR first. Handoff
the frozen narrow source to A; local CLI checks are separate from verified owner
play, human acceptance and main integration. The final aggregate remains required
after any demonstrated resource-routing repairs, with a coordinated Mac
reservation or independent cloud runner. No overlapping full/browser suites.

## Triage continuation

A has been asked for original owner 957.14-second / 43-failure evidence and exact
original C failing logs. `/private/tmp/a-systems30-delivery-final.md` was read as
a historical receipt, not a transcript or permission to publish `9aea1aba`.
Current public C source `20ef8d435f741e6b2797b65a162e919819f72581` has a successful
repository job `103790588425` in run `34782001355`, with browser shards queued at
inspection. This neither classifies old failures nor certifies unpublished C.
Read actual logs, reproduce affected files with bounded workers, retain original
results and route deterministic product failures to their owner. Obtain audit JSON
at the exact lockfile, verify primary advisory sources before proposing upgrades.
No cleanup, other-owner shutdown, release or held-material upload is authorized.

## LEARN

Unexpected problem: launcher output directed users to review fixtures while the
ordinary play route already existed; a validation-before-server owner command
then prevented any server start after test failure.
Root cause: destination guidance confused developer review with ordinary Play;
the owner command coupled play availability to unrelated full-suite results.
Recurrence mechanism: CLI assertions enforce ordinary Play before distinctly
labeled candidate/review links and exercise the shell's existing direct launch
