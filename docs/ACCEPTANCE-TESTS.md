...
record. It does not schedule anything. A separate response from the requested
reporter must accept the saved channel and terms. A current colleague must
separately accept the preparation assignment. Refusal, no response, the wrong
person, or family/household status without shared current work leaves schedule
and work history unchanged.

### PRODUCERS-003 — Preparation and feedback use actual knowledge

Prepared facts are derived only from saved knowledge records belonging to the
assigned adviser after the work item is ready. Unknown, other-person and future
knowledge is rejected. Feedback cannot exist before the story's actual
publication; its producer records the assigned adviser's media knowledge of
that publication and then the adviser's fallible interpretation, never a poll
or automatic effect.
