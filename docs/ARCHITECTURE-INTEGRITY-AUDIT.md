...
## Modular Character Foundation Audit — 2026-09-01

This audit applies D-053 to the presentation and art layers and rechecks
D-004, D-047, the frozen Stage 6 baseline, and later-stage gates.

| Concern                        | Disposition         | Evidence and rationale                                                                                                                                                                                                                                                                                                |
| ------------------------------ | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Primitive reuse                | Confirmed           | Identity and context selection use `SeededRng` keyed forks and `stableHash` from `src/simulation/`; no presentation hash, modulo selection, or second RNG exists. Components are ordinary manifest assets under the one existing release gate; the catalog ledger is a sibling manifest file, not a second registry.  |
| Closed set versus open content | Confirmed separated | Component kinds are a closed TypeScript union with kind-specific validation. Families, pose families, head orientations, anchor IDs, and slot IDs are validated content keys declared by the components and catalog, never engine enums.                                                                              |
| Determinism and lineage        | Confirmed frozen    | A generation's membership and definitions are hashed into its ledger signature; validation rejects edits and smuggled members. Identity pinned to a generation reproduces exactly after growth (300-seed proof); one fork per slot keeps slots independent.                                                           |
| Anchor identity                | Confirmed distinct  | Scene anchors (`SceneVisualAnchor`), the pelvis-hip-center root, and body-owned attachment anchors are separate types with separate owners. Anchors are metadata; no imagery carries marks. `attachmentSlots` is now the typed anchor contract rather than an empty tuple.                                            |
| Headless presentation          | Confirmed           | `character-components.ts` imports no React, DOM, Vite, Node, or ambient entropy; resolution never mutates the person, appearance, or `World`. The art validator and browser runtime share the module.                                                                                                                 |
| Fallback honesty               | Confirmed labeled   | The production catalog is an empty bootstrap; the fixture library holds draft definitions with no raster. A pose without body art fails closed; a slot without pose art is omitted; unreleased components are flagged, never substituted.                                                                             |
| Stage 6 and scope gates        | Confirmed absent    | No simulation record, schema, or `Person`/`World` change. No component art, generator, wardrobe library, normalization, head-angle generation, animation, engine, replacement of A01/B01, Slice F, campaign/election, or corpus PR change was made. Persisting a per-person pinned generation is deferred and stated. |

The 23-test focused contract suite and 5-test validator suite prove schema,
...
## Legislative Gameplay Core Audit — 2026-09-01

This audit applies D-056 to the simulation, presentation, and player layers and
rechecks D-002, D-004, D-006, D-019, D-026, D-047, D-050, and later-stage gates.

| Concern                          | Disposition          | Evidence and rationale                                                                                                                                                                                                                                                                    |
| -------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Open-set and example safety      | Confirmed separated  | Procedural stage kinds, vote dispositions, denominators, and rounding rules are closed unions with distinct validation. Jurisdictions, chambers, committees, floor stages, and thresholds are data in rule packs; no state name, seat count, or threshold is an engine constant.          |
| Overgeneric schema               | Confirmed typed      | `RuleValue<T>` is a three-state discriminated union, not a nullable field or metadata bag. A resolved negative, an unresolved rule, and an inapplicable concept are separately representable and separately readable.                                                                     |
| Semantic behavior                | Confirmed behavioral | Rule differences change the legal route, not a label: Nebraska runs three floor stages and offers no transmittal; Alaska reconsiders a veto in one 60-member sitting and refuses per-chamber forums; Kentucky sustains or overrides on a majority of each chamber's elected members.      |
| Primitive reuse                  | Confirmed            | Stable IDs, seeded worlds, append-oriented history, ordinary events, the future-due substrate, the snapshot codec, and the SQLite repository are all the existing ones. No second clock, no second history, no parallel identity.                                                         |
| Election anti-example            | Confirmed distinct   | Legislative voting records named members, an explicit denominator, a threshold fraction and a rounding rule. It shares no code or shape with election vote shares and tallies; integrity recomputes every count from the record's own dispositions.                                       |
| Derived rather than stored state | Confirmed            | A measure's phase, chamber, committee, floor stage, gate, and available steps are replayed from its actions against its rule pack. No status bucket is persisted.                                                                                                                         |
| Behavior versus rule             | Confirmed separated  | Rule packs carry only formal rules. How members decide is authored per scenario and labeled as such in vote provenance; no step applies a numeric modifier to a tally. The bargaining seam is left open and empty.                                                                        |
| Fallback honesty                 | Confirmed labeled    | Unresolved rules stay unresolved: gubernatorial inaction, effective dates, quorum fractions, and Alaska's carry-over are surfaced to the player as unsettled rather than defaulted, and recording an adjournment death is refused where that rule is unknown.                             |
| Headless core                    | Confirmed            | The simulation modules import no React, DOM, persistence, or ambient entropy; the boundary suite covers the new files.                                                                                                                                                                    |
| Player language                  | Confirmed            | A dedicated test asserts no internal vocabulary reaches any player-facing string, in every scenario.                                                                                                                                                                                      |
| Stage and scope gates            | Confirmed absent     | No campaign, election, electorate, lobbying, caucus, public-opinion, appropriations, budgeting, judicial, or first-session change. Conference committees, concurrence, live deadlines, line-item vetoes, and the other forty-seven states are named as unimplemented rather than stubbed. |

## Legislative Gameplay Core — Post-Audit Repair — 2026-09-02

An independent audit of PR #53 at `7ad3623` returned REPAIR-FIRST with fourteen
confirmed defects. It found the architecture sound and the claims ahead of the
behavior. This entry records what changed and what remains true.

| Finding                                         | Disposition | Evidence and rationale                                                                                                                                                                                                                                                                                                               |
| ----------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1 Integrity accepted illegal histories         | Repaired    | `replayMeasure` validates each action against the state immediately before it, its named chamber/committee/stage, and the rule authorizing it; nothing may follow a terminal action. `assertLegislationIntegrity` throws on the first violation.                                                                                     |
| D2 Second-chamber amendments skipped agreement  | Repaired    | An amendment adopted after transmittal routes the measure to `awaiting-concurrence` in its origin chamber. `recordConcurrenceVote` decides it on the existing threshold; refusal ends the bill. Unchanged text still goes straight to enrollment. Conference remains unimplemented.                                                  |
| D3 Enactment checked "ever signed"              | Repaired    | `recordEnactment` asserts the exact `awaiting-enactment` phase and refuses a measure already resolved. Integrity independently rejects two resolutions or one contradicting the history.                                                                                                                                             |
| D4 Module-global session identity               | Repaired    | `nextMeasureStableKey` derives keys from the measure's own recorded history. The counter and its reset are gone. Repeated save/reload with several amendments, and two worlds played in step, are covered by regressions.                                                                                                            |
| D5 Executive outcomes as player choices         | Repaired    | The step vocabulary is acts and requests, not outcomes. At the executive stage the only step is a neutral wait, rendered under "Out of your hands"; the outcome appears afterwards. The test asserting a label/result contradiction is gone.                                                                                         |
| D6 `unknown` permitted action                   | Repaired    | Amendment permission and presentment go through `requireKnown`, and a step whose controlling rule is not known-true is not offered at all.                                                                                                                                                                                           |
| D7 `not-applicable` permitted adjournment death | Repaired    | `recordAdjournmentDeath` goes through `requireKnown`. No pack currently resolves the rule, so all three refuse, each in its own words.                                                                                                                                                                                               |
| D8 Recommendation conflated with motion result  | Repaired    | `CommitteeDisposition` is a union of `reported` with a recommendation and `not-reported`. A carried unfavorable or no-recommendation report reaches the floor; a failed motion is its own event.                                                                                                                                     |
| D9 Separate-day rules had no force              | Repaired    | The earliest eligible date is derived on replay, the writer refuses a premature stage, and the player is offered a truthful wait on the world clock. Nebraska's three stages now land on three dates, shown in the vote record.                                                                                                      |
| D10 Rule-pack source errors                     | Repaired    | Every pack re-audited against primary text; each Kentucky chamber cites its own rules; Alaska's window/inaction move to Art. II Sec. 17 and its effective date to Sec. 18; Uniform Rule 22 is no longer misused; Nebraska's "most bills" is no longer "every bill". Evidence recorded in `docs/systems/legislative-rule-sources.md`. |
| D11 Executive timing could contradict the world | Repaired    | The writer no longer accepts a date; disposition, action and event all carry `world.currentDate`, and integrity checks they agree and follow presentment. The always-null `actionDeadline` field is removed rather than left implying a value.                                                                                       |
| D12 Money-bill threshold silently substituted   | Repaired    | Gate projection and integrity both use `requireKnown`. An unresolved heavier threshold is told to the player as unresolved, never shown as the ordinary one.                                                                                                                                                                         |
| D13 Alaska narrated an unimplemented veto       | Repaired    | The scenario's rationale is a whole-bill veto. Line-item veto stays unimplemented and unnarrated.                                                                                                                                                                                                                                    |
| D14 `members-elected` meant authorized seats    | Repaired    | The denominator is documented as members actually elected; `electedMembersFor` accepts a real count bounded by the chamber's seats; integrity treats seats as a ceiling. A vacancy regression fixes 20 of 38 at full strength and 19 of 36.                                                                                          |

Adjacent player-language repairs, within the same files: institution-commanding
button labels became requests, motions and waits; the doubled committee label
is gone and the committees are named ones from each chamber's own rules; the
source-workbench voice ("not settled in our sources") is out of player prose
and a known rule is now stated as world fact; the vote record shows dates.

Still true from the 2026-09-01 entry: derived-not-stored position, election
anti-example separation, primitive reuse with no second clock, headless core,
...
about intake determinism, provenance, primitive reuse and fail-closed behavior
is unchanged.
