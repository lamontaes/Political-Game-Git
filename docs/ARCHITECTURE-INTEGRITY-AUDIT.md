...
| Behaviour versus rule            | Confirmed separated   | Rule packs carry only formal rules. How members decide is authored per scenario and labelled as such in vote provenance; no step applies a numeric modifier to a tally. The bargaining seam is left open and empty.                                                                       |
| Fallback honesty                 | Confirmed labelled    | Unresolved rules stay unresolved: gubernatorial inaction, effective dates, quorum fractions, and Alaska's carry-over are surfaced to the player as unsettled rather than defaulted, and recording an adjournment death is refused where that rule is unknown.                             |
...
behaviour. This entry records what changed and what remains true.

| Finding                                         | Disposition | Evidence and rationale                                                                                                                                                                                                                                                                                                               |
| ----------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1 Integrity accepted illegal histories         | Repaired    | `replayMeasure` validates each action against the state immediately before it, its named chamber/committee/stage, and the rule authorising it; nothing may follow a terminal action. `assertLegislationIntegrity` throws on the first violation.                                                                                     |
| D2 Second-chamber amendments skipped agreement  | Repaired    | An amendment adopted after transmittal routes the measure to `awaiting-concurrence` in its origin chamber. `recordConcurrenceVote` decides it on the existing threshold; refusal ends the bill. Unchanged text still goes straight to enrolment. Conference remains unimplemented.                                                   |
...
| D8 Recommendation conflated with motion result  | Repaired    | `CommitteeDisposition` is a union of `reported` with a recommendation and `not-reported`. A carried unfavourable or no-recommendation report reaches the floor; a failed motion is its own event.                                                                                                                                    |
...
| D14 `members-elected` meant authorised seats    | Repaired    | The denominator is documented as members actually elected; `electedMembersFor` accepts a real count bounded by the chamber's seats; integrity treats seats as a ceiling. A vacancy regression fixes 20 of 38 at full strength and 19 of 36.                                                                                          |
