...
The `repository` check fails on this branch's head, and the failure belongs to
the base branch rather than to this work. `prettier --check .` exits 2 on 87
files: 52 under `public/data/state-voting/v1`, 29 scraped pages under
`docs/reference/regional-opening/raw`, 6 docs elsewhere. All 87 are
byte-identical to the base commit; none of the files this branch changes is
among them. The 19 hard errors are HTML parse failures in the captured pages —
an unquoted `href= /m/directory`, several unbalanced `</div>` — which no
formatting pass can repair.

The scraped pages are external captures of the same kind as `data/source` and
`art/authoring/`, which `.prettierignore` already excludes for exactly this
reason. Reformatting them would be actively wrong: their retrieval receipts
record each page's byte length and checksum, so a reformat breaks every recorded
hash. The exclusion is the repair, and it belongs to whoever owns those files.
