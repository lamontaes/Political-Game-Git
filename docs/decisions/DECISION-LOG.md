...
Store reusable effect response semantics in a small world catalog and committed cause-to-metric relationships as separate `EffectActivationRecord` history. Run B supports only exact linear and bounded ease-out response curves with typed magnitude/direction, onset/ramp/maturity/end, optional compatible threshold and target bound, open realization key, and stable metric/scope identity. It adds no equation DSL, serialized callback, arbitrary payload, or silent field mutation. Evaluation is explicit and inspectable. Only the contained `recordEvaluatedMetricState` seam may commit a result through the existing metric writer, with the baseline and contributing activation IDs preserved as provenance.
