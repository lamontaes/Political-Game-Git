...
Require a separate `PolicyRealizationRecord` before actual consequences exist. Blocked or untriggered realization creates no actual cause/effect. Full or partial realization creates one actual Run B causal process and compatible Run B activations with the accepted point-target or exact interval-total magnitude basis; canonical metric truth remains a later explicit baseline-plus-effect write. Delayed realization reuses the one Run A due-item handler seam and adds no recurrence or scheduler.
