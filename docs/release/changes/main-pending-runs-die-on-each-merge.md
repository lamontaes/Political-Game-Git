...
executing, and a run still waiting for a slot is cancelled outright when the
next merge enters its concurrency group. Three consecutive main runs were
measured being cancelled with zero jobs allocated. The second is that the CI
