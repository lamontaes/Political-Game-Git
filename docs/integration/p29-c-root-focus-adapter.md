...
4. `PersonSceneActionMenu.onDismiss` requests scene-invoker focus and dispatches
   the existing `close-action-menu` action. The focused menu consumes Escape,
   so the document handler cannot dismiss another layer with the same key.
   This is the narrow repair for H's current-candidate body-focus finding;
   other shell Escape ordering and workspace ownership remain unchanged.
