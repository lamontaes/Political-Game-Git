...
Base: current `main` at the DISTRICTS13 publication SHA. Fiscal query/work files on this branch match Fiscal166; keep one copy if LAND already has them. This lane does not hold Fiscal166 for a fabricated picker membership journey.
...
A new life started in a Census place that the 2024 relationship files show wholly inside one state-legislative district receives that membership automatically. Lexington-Fayette is split and does not.
...
- Split places, counties, statewide homes, and congressional place joins stay unknown or conflicting.
- This is not the completed geographic system.
- Ordinary player-route visual acceptance remains A's after mount.

## Proof owned here

Focused tests in `src/districts/query.test.ts` and `src/presentation/districts13.test.ts`: refusal boundaries, interrupted residence, old-save UNKNOWN, picker-is-not-membership, whole-place Adak join, ordinary filing/election/fiscal without a forced win. Independent review of the membership boundary is distinct from this author run.
