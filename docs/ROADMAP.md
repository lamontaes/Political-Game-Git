...
Runs A and B deliberately contain no automatic economy tick, policy forecast or implementation, generalized incident selection/chain engine, mortality/incapacity/evidence system, law/institutions/elections, media ecology, territory-specific content, or player-facing UI. Metric state, observations, causal attribution, and subjective knowledge remain distinct. Future due items remain the only future-transition mechanism and contain no opaque payload or recurrence language. See D-042, D-043, the completed Run A/Run B plans, [World State, Observations, and Future Transitions](systems/world-state-and-observations.md), and [Causal Mechanisms, Lightweight Economy, and Fiscal Continuity](systems/causal-economy-and-fiscal.md).
