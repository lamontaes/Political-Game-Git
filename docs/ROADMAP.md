...
Runs A through C deliberately contain no automatic economy tick, generalized incident selection/chain engine, mortality/incapacity/evidence system, law/institutions/elections, automatic forecast generator, media ecology, territory-specific content, or player-facing UI. Forecast, canonical metric state, observations, causal attribution, realization, and subjective knowledge remain distinct. Future due items remain the only future-transition mechanism and contain no opaque payload or recurrence language. See D-042 through D-044, the completed Run A–C plans, [World State, Observations, and Future Transitions](systems/world-state-and-observations.md), [Causal Mechanisms, Lightweight Economy, and Fiscal Continuity](systems/causal-economy-and-fiscal.md), and [Quantitative Policy Semantics, Baselines, and Implementation](systems/quantitative-policy.md). Run D generalized incidents is the next candidate; Stage 6 remains **IN PROGRESS**.
