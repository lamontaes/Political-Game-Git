...
This branch stops strictly at deterministic trait resolution. The actual _image composition_ (e.g., 2D bounds, crop coordinates, transparent canvas handling, layer depth sorting) is outside the scope of this architecture and should be handled by an independent generic `Asset-QA` / compositing service later.
